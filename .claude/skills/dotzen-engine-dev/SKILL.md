---
name: dotzen-engine-dev
description: Use this skill when implementing or modifying dotzen's own engine code (the TypeScript CLI package, HCL parsing integration, rule evaluation logic, version-pinning enforcement, or CI/pre-commit distribution mechanics) as opposed to writing a governance spec file. Triggers on work inside packages/cli/src/, bin/dotzen.js, the HCL parser subprocess wrapper, or requests to add a new resource type, evaluation condition, or output format to dotzen itself. For editing .zen/spec.ts rule content, use dotzen-spec-authoring instead.
---

# dotzen Engine Development

You are working on dotzen's own implementation, not a consumer's spec
...
4. **Never hardcode or default to `@latest` anywhere** — not in
   ...
   path that resolves a version must go through the `dotzen.json`
...
pipeline of total functions `(input) => Result<Output, DotzenError>`
(readDotzenJson → enforceVersion → loadSpec / parseTf → normalize →
...
5. **The engine never sees the parser's raw output type.** The `hcl/`
   layer normalizes into dotzen's own `NormalizedResource` model; the
...
the filesystem for HCL — the CLI must read `dotzen.json` (if present)
...
(`AwsResource` enum, consumed via `dotzen-spec-authoring`) and the
...
**dotzen is developed test-first (TDD, red → green → refactor).** Full
...
a total `(input) => Result<Output, DotzenError>` and `evaluate` consumes
...
- Every new rule condition type (deny-ingress, must-have-attribute,
  ...
  red step).
- Test the `dotzen.json` version-mismatch path explicitly — this is a
