---
name: pluvian-spec-authoring
description: Use this skill whenever writing, editing, or reviewing a pluvian governance spec file (.pluvian/spec.ts), or when a user asks to add/modify a governance rule for Terraform infrastructure, or asks what rules a pluvian spec contains. Triggers on mentions of ".pluvian/spec.ts", "pluvian rule", "governance rule", "add a rule for", or requests to express a security/compliance/tagging/cost policy as code for Terraform. Do not use this skill for pluvian's own engine implementation code (parsing, evaluation logic) — use pluvian-engine-dev for that instead.
---

# pluvian Spec Authoring

You are writing or editing `.pluvian/spec.ts` — the file a **security
...
A spec whose rules default to `Warn` trains developers to ignore pluvian
...
`.exhaustive()` internally (see `pluvian-engine-dev` skill), the
