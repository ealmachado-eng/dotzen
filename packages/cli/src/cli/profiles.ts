/**
 * Profiles + presets for `dotzen init --profile <name>` / `--presets <list>`.
 *
 ...
 *
 * This module is the SINGLE SOURCE OF TRUTH for both `dotzen init` (which
...
const renderImports = (imports: readonly string[]): string => {
  ...
  if (imports.length <= 5) {
    return `import { ${imports.join(', ')} } from '@dotzen/dotzen'`
  }
  return `import {\n  ${imports.join(',\n  ')},\n} from '@dotzen/dotzen'`
}
...
const defaultInitHeader = (
  ...
  return `/**\n * dotzen spec — generated by \`dotzen init\` (${composed}).\n * Edit presets + rules to fit your org, then run \`npx @dotzen/dotzen check\`.\n */`
}
...
/** Compose the full `.zen/spec.ts` content from a profile + extra presets. */
