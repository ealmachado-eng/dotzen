# What dotzen does — and doesn't

> **Audience:** everyone new to dotzen — security reviewers, spec authors, platform engineers. A 5-minute orientation before you read the [tutorial](./tutorial.md) or the [rule reference](./reference/rules/all-rules.md).

## What dotzen is

**dotzen is governance-as-code for Terraform.** It statically analyzes your `.tf` files (HCL) against a readable, strongly-typed rule spec written in TypeScript (`.zen/spec.ts`) and reports security, tagging, and compliance violations — **before** `terraform plan` or `apply`. It is designed for the world where AI code-generation tools produce Terraform that compiles but quietly violates your organization's policies.
...
2. **Zero-install.** `npx @dotzen/dotzen check` pulls a pinned version and runs. No local install, no native binary, no macOS Gatekeeper / Windows SmartScreen prompts. The HCL parser is the official HashiCorp HCL compiled to WASM (`@cdktf/hcl2json`).
3. **Honest, not loud.** When dotzen can't statically resolve a value (a `var` with no default, a computed expression), it reports **"could not evaluate"** — never a guess. A false positive is worse than an honest gap.
...
- **Inline ignore directives** — suppress a finding on a specific block with a documented reason: `# dotzen:ignore: bastion host — SSH is intentional behind a CIDR allowlist`. Auditable, not silent.
...
- **Ungoverned-coverage telemetry** — resource types dotzen recognizes but that have no rule surface as `ungoverned`, so you can see your coverage gaps rather than assume "no finding = compliant".

## What dotzen does **not** do

Honest boundaries — knowing these prevents misuse reports and sets the right expectations.

- **No `terraform plan`.** dotzen never runs Terraform, never touches your cloud, never reads state. Everything is derived from HCL. Anything that requires runtime values (a data source lookup, a `terraform plan` output, a conditional that depends on cloud state) is reported as **could-not-evaluate**, not guessed.
...
The CI/CD governance stack, and where dotzen lives in it:

| Layer                         | Tool                            | What it catches                                                                      |
| ----------------------------- | ------------------------------- | ------------------------------------------------------------------------------------ |
| **Style / formatting**        | `terraform fmt`, `tflint`       | Style, deprecated syntax, provider quirks                                            |
| **Static policy (this tool)** | **dotzen**                      | Security/compliance/tagging violations in HCL — _resource-aware, organization-aware_ |
...
dotzen sits between style-checks and plan-time policy: **structural, cloud-aware governance of the HCL itself**, credential-free, in CI/pre-commit. It's complementary to OPA/Sentinel (which need a plan) and to gitleaks (which scans everything, not just structured secrets on resources).

### What makes dotzen different from Checkov / TFSec / KICS

Those tools ship a fixed, opinionated rule set baked into the binary. dotzen's rules are **authorable TypeScript you control** — your security team writes (or curates) the policy, in a DSL readable enough to review in a change request. The presets (CIS / PCI / NIST) are a starting point you compose and extend, not a black box. And dotzen's could-not-evaluate discipline means it won't false-positive on a value it can't resolve — it tells you honestly.
